#git commit -m 'my change' ; git push
...
# data_pipeline_paramter = {
#     "topic_name": f'egress_gnb0',  # Name of the topic from where to get the data
#     "bootstrap_server": '163.173.170.88',  # Ip address of the borker (eNDBF)
#     "consumer_group": 'preprocessed_cpu_group',
# }
# producer_sleep_time_interval = 0.2
...
# }
        ...
        self.current_epochs_before_aggregation = 0
    ...
    def train_keras_model(self, train_data_segment, resource_name, k_model):
        ...
        print_cyne(f'{resource_name} Starting ->[Round  : {self.current_round+1}/{self.total_rounds}]')
        print_cyne(f'{resource_name} Starting ->[Inter round  : {self.current_epochs_before_aggregation+1}/{self.epochs_before_aggregation}]')
        ...
        #Change the function to return the model rather than the weights
    ...
    def inter_round_data_analysis(self, x_train, resource_name, k_model, ds_mnt):
        ...
        # print_purple(f"MSE:{sys.getsizeof(train_MSE) }")
        ...
        #dir_path = self.analysis_data_dir_path+"/"+self.layer+"/"+self.rsc_target+"/"+self.output_folder
        #filename =  'trained_model_'+str(resource_name)+'_'+str(round)+'_'+str(self.current_round)+'.h5'
        #do.save_globaltrained_model(k_model,dir_path,filename)
    ...
    def load_client_dataset(self, training_data_splits_path, rsc_target, day_index, client_idx, no_training_samples):
        ...
        if self.check_file_exist(training_data_dir_path):
            ...
            # self.segment_size = trai  ning_data.shape[0] / self.total_rounds
    ...
    def produce_json_result(self, resource_name):
        ...
        training_settings = {
            ...
            #"data_shape": f"({str(self.total_data_samples)}, {str(self.training_data_shape[resource_name][1])},{str(self.training_data_shape[resource_name][2])})",
        ...
        # modified on Sep2023 .. check server comments
        ...
        #fullpath_qntl = os.path.join(dir_path, f'mse_{str(self.client_id)}.json')
        #do.save_json_to_file({"MSEs":self.incremental_MSE.tolist()}, fullpath_qntl)
        #os.chmod(fullpath_qntl, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)
    ...
    def do_training(self, db_splits_count, resource_name, init_model):
        ...
        for day in range(1, db_splits_count + 1):
            ...
            #TODO: uncomment if its NOT RealTimje data
            ...
            # CUT THE LAST 4000 SAMPLES OF DAY 14
           # if day == db_splits_count:
           #     print_orange("---- LAST ROUND ----")
           #     training_data = training_data[:-4000]
            ...
            for round_ in range(self.rounds):
                ...
                while epoch_  <= self.epochs_before_aggregation:
                    ...
                    epoch_ =  epoch_ +1
                    count_discarded_data = 0
                #send the model and get response -- save it in g_model_
                #We change the inpout parameter from  local_model to function to local_model.get_weights()
                #g_model_ = self.transmit_local_model(resource_name=resource_name, local_model=local_model)
