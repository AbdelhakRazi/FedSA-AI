...
WAIT_FOR_DS_SPLITS = 6  * 60  # 6  minutes
WAIT_BEFORE_CHECK = 5 * 60  # 5 minutes
...
class Client:
    ...
    def transmit_local_model(self, resource_name, local_model):
        ...
        self.print[resource_name](f'At {lcl_t} {resource_name} local trained model transmit finish  ')
        self.model_exchange_time = time.time() - ts
        self.get_fl_global_model(resource_name, g_model)
    ...
    def train_global_model(self, resource_name, g_model):
        ...
        local_model = self.select_training_frmrk(resource_name, self.global_model[resource_name])
        return local_model

    def select_training_frmrk(self, resource_name, g_model):
    ...
    def train_keras_model(self, resource_name, k_model):
        if self.current_round <= self.total_rounds - 1:
            ...
            if inter_round == 0:
                ...
                #Just to make more assurance the PREVIOUS dataset file split is no longer exist
                #split_path = f'{self.training_data_dir_path}/{self.rsc_target.lower()}_{day-1}_{self.client_id}'
                #if os.path.exists(split_path):
                #    print(f"Warning: remaining file {split_path} not deleted")
                #    os.remove(split_path)
                #else
                #    print(f"{split_path} is already deleted")
                ...
                self.training_data[resource_name] = self.load_client_dataset(
                ...
                is_file_loaded = True
                self.total_data_samples += self.training_data[resource_name].shape[0]
                e_time = time.strftime("%H:%M:%S", time.localtime())
                time_fld = get_formatted_elapsed_time(t_fl)
                file_loaded_dic[day] = {"start_time": s_time, "end_time": e_time, "duration": time_fld}


            print_blue("training data shape: ", self.training_data[resource_name].shape)

            self.training_data_shape[resource_name] = self.training_data[resource_name].shape

            train_data = self.training_data[resource_name]
            eval_data = train_data #self.evaluation_data[resource_name]
            ...
            if len(self.running_mode.keys()) > 0:
                if self.running_mode["mode"].lower() == "batch":
                    ...
                    segment_size = self.segment_size  # self.running_mode["batch_size"]
                    train_data_segment, self.training_data[resource_name]  = get_data_batch(train_data, segment_size, inter_round, self.rounds )
            ...
            print_purple(f"training_data:{sys.getsizeof(self.training_data[resource_name])/(1024**2)} MB")

            print_purple(f"train_data_segment:{sys.getsizeof(train_data_segment)/(1024**2)} MB")
            ...
            dtan = {"client_id": self.client_id,
                    ...
                    "DS_split_file_loaded": "No" if not is_file_loaded else file_loaded_dic
                    }
    ...
    def ready_to_start(self, resource_name):
        ...
        if response.response:
            ...
            loaded_model.compile(optimizer=optimizer, loss='mean_squared_error', metrics=['mse'])

            loaded_model.set_weights(g_model['weights'])

            self.global_model[resource_name] = loaded_model

            local_model = self.select_training_frmrk(resource_name, loaded_model)
            # ----------------------------------------------
            self.transmit_local_model(resource_name, local_model)
        else:
