...
def train(client_id, client_str, all_dataset, train_data_cpu, test_data_cpu, dir_path, client_file_index, client_count,
          running_mode, set_rounds=0,  data_lake_param={}, rsc_target="unknown", segement_size=0):
    ...
    print(x_train.shape)
    ad = client.send_data(rsc_name_dir[rsc_target], x_train, y_train, test_data_cpu, segement_size=segement_size, client_count=client_count)
...
def collect_daily_data(_filepath, _no_samples, no_of_participants):
    dataset = get_data_from_file(_filepath)
    ...
    dataset = dataset.X_train_x[:_no_samples]

    # _no_samples = dataset.shape[0]
    ...
    all_dataset = split_dataset(no_clients=no_of_participants, train_data=dataset)

    return all_dataset, test_data
...
if __name__ == '__main__':
    ...
    no_samples = config["data"]["no_samples"] # used to have limited numbers of samples from the whole DB, set it to a large NO. to have all samples in a DS
    ...
    train_data_dir_path = f'{ROOT_DIR}/{train_data_dir_path}/{rsc_target}' # not the file name (cpu,memory, network, disk or radio)
    ...
    all_dataset, test_data = collect_daily_data(train_data_dir_path, no_samples, NUMBER_OF_PARTICIPANTS)
    ...
    segement_size = client_dataset.shape[0] / 24 # Each batch is one hour
    ...
    if mode.lower()=="batch":
        ...
        if settings == "ROUND":  # Adjust Rounds and Fix Batch size
            sp_rounds = math.ceil(client_dataset.shape[0] / segement_size)
            print("Current rounds setting: ", end="")
            print_orange(sp_rounds)
        elif settings == "BATCH":  # Adjust Batch size and Fix Rounds
            segement_size = math.ceil(client_dataset.shape[0] / sp_rounds)
            running_mode['batch_size'] = segement_size
            print("Current batch size setting : ", end="")
            print_orange(segement_size)
        elif settings == "TIME":
            pass
