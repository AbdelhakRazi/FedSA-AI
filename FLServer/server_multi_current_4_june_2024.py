...
class FLServer(FederatedLearningServicer):
    ...
    def aggregate_local_models(self, resource_name):  # independent thread to
        ...
        if self.total_rounds > self.current_round[resource_name]:
            ...
            force_aggregation = client_count > 1 \
                                and (time.time() - self.do_wait) > MAX_WAITING_TIME_FOR_CLIENT_CONTRIBUTION \
        ...
        self.save_trained_model(self.global_model, self.trained_model_folder)
        client_count = len(self.client_with_local_model[resource_name])
        #self.transmit_saved_model(client_count,model_file_path)
        print("=" * 80)    
            # self.init()
        
    
    
    def save_trained_model(self, model, traind_model_folder):
        
        pcolors.print_orange("\tsaving trained model")
        file_name = f'{self.rsc_target}_{self.folder_string}.h5'
        fldr_name = f'{traind_model_folder}/{self.layer}'
        fldr = Path(fldr_name)
        print("traind_model_folder=", traind_model_folder)
        if not fldr.exists():
            fldr.mkdir(parents=True)
        model.save(Path(f'{fldr}/{file_name}'))
        # os.chmod(f'{fldr}/{file_name}', stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)
        pcolors.print_orange(f'{file_name} saved successfully at location: {fldr_name}')
        file = f'{fldr_name}/{file_name}'
        print("client_to_send_global_model: ",self.client_to_send_global_model)
